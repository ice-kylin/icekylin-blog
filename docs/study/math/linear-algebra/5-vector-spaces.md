...
虽然 $R^{n}$ 是最重要的向量空间，但我们更关心 $R^{n}$ 内的向量空间。

这些向量空间满足既定规则，又无需包涵所有向量。我们将其称为原向量空间的子空间。

上述的两个例子中的向量集合都不构成向量空间（自然也不构成子空间），而下面的例子则构成向量子空间。
...
其中一种方法是通过列向量构造。给定 $m \times n$ 矩阵 $A$，其列向量属于 $R^{n}$，这些列向量所有的线性组合构成了 $R^{n}$ 中的一个子空间，即矩阵 $A$ 的列空间，记为 $C \left(A \right)$。
...
观察 $A$ 的各列，它们均为 $R^{3}$ 中的向量。用这些列构造 $R^{3}$ 的子空间，也就是这些列向量所有的线性组合构成了 $R^{3}$ 中的一个子空间，在图像上此处的子空间是一个过原点的平面：

![r3-1](./5-vector-spaces/r3-1.png)

::: tip 💡 tip

在更高维度的情况下就没法作出图像了。例如在 $R^{10}$ 中，作出 $R^{10}$ 中 $5$ 个向量的线性组合，可能最后得到的就是十维空间中五维“平面”或更低维“平面”甚至是穿过原点的直线（当这 $5$ 个向量共线）、甚至只是原点（仅仅包含一个有着 $10$ 个分量的零向量）之类的某种子空间。

:::

在下一小节中则将在列空间和子空间的基础上理解 $Ax = b$。
